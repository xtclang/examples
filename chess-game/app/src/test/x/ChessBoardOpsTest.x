module ChessBoardOpsTest {
    ...
    class BoardOperationsTests {
        ...
        void shouldFindAndCountPieces() {
            val array = BoardUtils.cloneBoard(ChessLogic.defaultBoard());
        ...
        void shouldApplyAndCopyMoves() {
            ...
            val copied = BoardOperations.boardWithMove(board, 12, 28);
            assert copied[12] == '.';
            assert copied[28] == 'p';
            assert board[12] == 'p';
        }
        ...
        void shouldFindMultiplePieces() {
            val array = BoardUtils.cloneBoard(ChessLogic.defaultBoard());
            val pawns = BoardOperations.findPieces(array, 'P', Color.White);
            assert pawns.size == 8;
            val knights = BoardOperations.findPieces(array, 'N', Color.White);
        ...
        void shouldReportMaterialBalanceWithMissingPieces() {
            ...
            val balance = BoardOperations.calculateMaterialBalance(board);
    ...
    class CastlingAndContextTests {
        ...
        void shouldUpdateCastlingRightsAndFormatFEN() {
            val rights = CastlingManager.defaultRights();
            assert CastlingManager.toFEN(rights) == "KQkq";
            assert CastlingManager.toFEN(CastlingManager.noRights()) == "-";

            val afterWhiteKingMove = CastlingManager.updateRights(rights, 'K', 60, 62);
            ...
            val afterRookCapture = CastlingManager.updateRights(rights, 'P', 48, 63);
            assert !afterRookCapture.whiteKingside;
        }

        @Test
        void shouldRespectMoveContextCastleChecks() {
            val noRights = new MoveContext();
            assert !noRights.canCastle(Color.White, True);

            val whiteRights = new MoveContext(new CastlingRights(True, False, False, False));
        ...
        void shouldUpdateRightsWhenRookMoves() {
            ...
            val afterKingsideRookMove = CastlingManager.updateRights(rights, 'R', 63, 55);
            ...
            val afterQueensideRookMove = CastlingManager.updateRights(rights, 'R', 56, 48);
        ...
        void shouldUpdateRightsForBlackKingMove() {
            val rights = CastlingManager.defaultRights();
            val afterBlackKingMove = CastlingManager.updateRights(rights, 'k', 4, 5);
        ...
        void shouldCheckCanCastleForBothColors() {
            val full = CastlingManager.defaultRights();
            ...
            val none = CastlingManager.noRights();
        ...
        void shouldCreateMoveContextWithEnPassant() {
            val ctx = new MoveContext(new CastlingRights(), "e3");
