 ...
 * It provides a RESTful API for managing chess games with automated opponent moves.
 * 
 * Key features:
 * - Turn-based chess gameplay with simplified rules (no castling, en-passant, or check detection)
 * - Automated opponent (Black player) with AI-driven move selection
 * - Game state persistence using the chess database schema
 * - RESTful API endpoints for moves, game state, and game reset
